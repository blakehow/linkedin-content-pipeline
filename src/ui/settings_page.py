...
def show():
    ...
    tabs = st.tabs(["User Profile", "Brand Profiles", "Content Preferences", "Idea Categories", "AI Configuration", "Pipeline Stages"])
    ...
    with tabs[5]:
        st.subheader("Pipeline Stage Configuration")
        st.markdown("Customize the names and descriptions of the 3 content versions generated by the pipeline.")
        ...
        with st.form("pipeline_stages"):
            ...
            if st.form_submit_button("Save Pipeline Stages", type="primary"):
                # Validation
                if not all([stage1_name, stage2_name, stage3_name]):
                    st.error("All stage names are required")
                elif len(set([stage1_name, stage2_name, stage3_name])) < 3:
                    st.error("Stage names must be unique")
                else:
                    settings.content_version_names = [stage1_name, stage2_name, stage3_name]
                    settings.content_version_descriptions = [stage1_desc, stage2_desc, stage3_desc]
                    storage.save_settings(settings)

                    st.success("Pipeline stages updated! The new names will be used in future content generation.")
                    st.rerun()

        st.markdown("---")
        st.markdown("### Current Stage Configuration")

        col1, col2, col3 = st.columns(3)
        with col1:
            st.markdown(f"**{settings.content_version_names[0] if len(settings.content_version_names) > 0 else 'Bridge'}**")
            st.caption(settings.content_version_descriptions[0] if len(settings.content_version_descriptions) > 0 else "")
        with col2:
            st.markdown(f"**{settings.content_version_names[1] if len(settings.content_version_names) > 1 else 'Aspirational'}**")
            st.caption(settings.content_version_descriptions[1] if len(settings.content_version_descriptions) > 1 else "")
        with col3:
            st.markdown(f"**{settings.content_version_names[2] if len(settings.content_version_names) > 2 else 'Current'}**")
            st.caption(settings.content_version_descriptions[2] if len(settings.content_version_descriptions) > 2 else "")
